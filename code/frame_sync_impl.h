...
namespace gr
{
  namespace lora_sdr
  {
    ...
    private:
      ...
      std::vector<gr_complex> frame_download;//保存帧信号
      std::vector<double> info;//保存帧信息
      std::string m_frame_path;//帧信号保存path
      std::string m_frame_info_path;//帧信息保存path
      ...
      uint32_t temp_index;//前缀点数
      uint32_t payload_index;//总点数
      uint32_t quarter_index;//0.25的点数
    ...
    public:
      frame_sync_impl(uint32_t center_freq, uint32_t bandwidth, uint8_t sf, bool impl_head, std::vector<uint16_t> sync_word, uint8_t os_factor, uint16_t preamb_len,std::string frame_path,std::string frame_info_path);
      ~frame_sync_impl();
