...
namespace gr
{
    namespace lora_sdr
    {
        ...
        frame_sync::sptr frame_sync::make(uint32_t center_freq, uint32_t bandwidth, uint8_t sf, bool impl_head, std::vector<uint16_t> sync_word, uint8_t os_factor, uint16_t preamble_len)
        {
            //std::cout<<
            //return gnuradio::get_initial_sptr(new frame_sync_impl(center_freq, bandwidth, sf, impl_head, sync_word, os_factor, preamble_len,"/mnt/hgfs/lora_dataset/raw_frame_test_dataset_day_4/NLOS/A/uno_3/","/home/czc"));
            return gnuradio::get_initial_sptr(new frame_sync_impl(center_freq, bandwidth, sf, impl_head, sync_word, os_factor, preamble_len,"/home/czc/gr-lora_sdr/","/home/czc/gr-lora_sdr/"));
        }

        /*
         * The private constructor
         */
        frame_sync_impl::frame_sync_impl(uint32_t center_freq, uint32_t bandwidth, uint8_t sf, bool impl_head, std::vector<uint16_t> sync_word, uint8_t os_factor, uint16_t preamble_len,std::string frame_path,std::string frame_info_path)
            : gr::block("frame_sync",
                        ...
                        gr::io_signature::make2(1, 2, sizeof(gr_complex), sizeof(float)))
        {
            ...
            m_frame_info_path=frame_info_path;
            ...
            //初始化
            frame_download.resize(30*m_samples_per_symbol);
            temp_index=0;
            quarter_index=0;
            payload_index=0;
        ...
        void frame_sync_impl::frame_info_handler(pmt::pmt_t frame_info)
        {
            ...
            if (m_invalid_header)
            {
                ...
                m_sto_frac = 0;
                //恢复到初始状态
                std::vector<gr_complex>().swap(frame_download);
                frame_download.resize(30*m_samples_per_symbol);
                temp_index=0;
                quarter_index=0;
                payload_index=0;
            ...
            else
            {
                ...
                //更改大小
                frame_download.resize((m_symb_numb+1)*m_samples_per_symbol+temp_index);
            }
        }
                                          ...
                                          gr_vector_void_star &output_items)
        {
            ...
            case DETECT:
            {
                //std::cout<<"DETECT state debug"<<std::endl;
                ...
                if (abs(mod(abs(bin_idx_new - bin_idx) + 1, m_number_of_bins) - 1) <= 1 && bin_idx_new != -1) // look for consecutive reference upchirps(with a margin of ±1)
                {
                    ...
                    if(symbol_cnt!=(int)(m_n_up_req))
                        //保存第symbol_cnt个symbol
                        memcpy(&frame_download[m_samples_per_symbol * (symbol_cnt-1)], &in[(int)(m_os_factor / 2)], m_samples_per_symbol * sizeof(gr_complex));
                ...
                else
                {
                    ...
                    //保存第一个symbol
                    memcpy(&frame_download[0],&in[(int)(m_os_factor / 2)], m_samples_per_symbol * sizeof(gr_complex));
                    symbol_cnt = 1;
                ...
                if (symbol_cnt == (int)(m_n_up_req))
                {
                    ...
                    //因为0～(m_os_factor / 2)的采样点在上一个操作中已经保存，故保存的采样点个数为(items_to_consume-(int)(m_os_factor / 2))
                    memcpy(&frame_download[m_samples_per_symbol * ((int)(m_n_up_req)-1)], &in[(int)(m_os_factor / 2)], (items_to_consume-(int)(m_os_factor / 2)) * sizeof(gr_complex));
                    temp_index=m_samples_per_symbol * ((int)(m_n_up_req)-1)+(items_to_consume-(int)(m_os_factor / 2));
                }
                else
            ...
            case SYNC:
            {
                ...
                case NET_ID1:
                {
                    ...
                    { // look for additional upchirps. Won't work if network identifier 1 equals 2^sf-1, 0 or 1!
                        ...
                        if (additional_upchirps >= 3)
                        {
                            ...
                            //保存items_to_consume个采样点
                            memcpy(&frame_download[temp_index], &in[0], items_to_consume * sizeof(gr_complex));
                            temp_index+=items_to_consume;
                        }
                        else
                        {
                ...
                case QUARTER_DOWN:
                {
                    ...
                    if (m_state != DETECT)
                    {
                        ...
                        //保存items_to_consume个采样点
                        memcpy(&frame_download[temp_index], &in[0], items_to_consume * sizeof(gr_complex));
                        quarter_index=items_to_consume;
                        temp_index+=items_to_consume;
                    }
                }
                }
            ...
            {
            	 //std::cout<<"SFO_COMPENSATION debug"<<std::endl;
                // transmit only useful symbols (at least 8 symbol for PHY header)
                if (symbol_cnt==0) payload_index=temp_index;//修改
                if (symbol_cnt < 8 || (symbol_cnt < m_symb_numb && m_received_head))
                {
                    ...
                    symbol_cnt++;
                    //保存items_to_consume个采样点
                    memcpy(&frame_download[payload_index], &in[0], items_to_consume * sizeof(gr_complex));
                    payload_index+=items_to_consume;
                }
                ...
                else
                {
                    ...
                    payload_index+=items_to_consume;
                    //保存一个帧的数据到文件
                    std::string frame_filepath=m_frame_path+std::to_string(frame_cnt);
                    //std::string frame_filepath=m_frame_path+"test_frame";
                    std::cout<<frame_filepath<<std::endl;
                    //std::string filepath="/home/czc/gr-lora_sdr/examples/dataset/test_frame/"+std::to_string(frame_cnt);
                    std::ofstream fout(frame_filepath, std::ios::binary);
                    fout.write((char*)&frame_download[0], payload_index*sizeof (gr_complex));
                    fout.close();
                    //保存帧信息
                    std::string frame_info_filepath=m_frame_info_path+std::to_string(frame_cnt);
                    //std::string filepath1="/home/czc/gr-lora_sdr/examples/dataset/test_frame_info/"+std::to_string(frame_cnt);
                    std::ofstream fout1(frame_info_filepath, std::ios::binary);
                    info.push_back(quarter_index);
                    info.push_back(temp_index);
                    info.push_back(m_cfo_int+m_cfo_frac);
                    fout1.write((char*)&info[0], 3*sizeof (double));
                    fout1.close();
                    //恢复到初始状态
                    std::vector<gr_complex>().swap(frame_download);
                    std::vector<double>().swap(info);
                    frame_download.resize(30*m_samples_per_symbol);
                    temp_index=0;
                    quarter_index=0;
                    payload_index=0;
